...
const STATUS_STYLES: Record<string, { bg: string; border: string; badge: string }> = {
    ...
    received: { bg: 'bg-white', border: 'border-l-amber-500', badge: 'bg-amber-100 text-amber-700' },
    preparing: { bg: 'bg-white', border: 'border-l-blue-500', badge: 'bg-blue-100 text-blue-700' },
    ready: { bg: 'bg-white', border: 'border-l-purple-500', badge: 'bg-purple-100 text-purple-700' },
...
export const OrderCard = memo(function OrderCard({ order, onStatusChange }: OrderCardProps) {
    ...
    const getButtonIcon = () => {
        switch (status) {
            case 'pending': return <CheckCircle className="mr-2 h-4 w-4" />
            case 'received': return <ChefHat className="mr-2 h-4 w-4" />
            case 'preparing': return <Bell className="mr-2 h-4 w-4" />
    ...
    const getButtonClass = () => {
        switch (status) {
            ...
            case 'received': return 'bg-indigo-600 hover:bg-indigo-700 text-white'
            case 'preparing': return 'bg-purple-600 hover:bg-purple-700 text-white'
            case 'ready': return 'bg-blue-600 hover:bg-blue-700 text-white'
