    ...
    const onSubmit = async (values: MenuItemFormValues) => {
        ...
        try {
            ...
            const insertMenuItem = async () => {
                return await supabase.from('menu_items').insert({
                    profile_id: user.id,
            ...
            let { error } = await insertMenuItem()

            if (error?.code === '23503') {
                console.log('Profile or Category missing, attempting to ensure profile...')
                await ensureProfileExists(user.id)
                const retry = await insertMenuItem()
                error = retry.error
            }
